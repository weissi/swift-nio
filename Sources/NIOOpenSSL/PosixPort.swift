...
@inline(__always)
internal func wrapSyscall<T: FixedWidthInteger>(where reason: String = #function, _ body: () throws -> T) throws -> T {
    while true {
        let res = try body()
        if res == -1 {
            let err = errno
            ...
            throw IOError(errnoCode: err, reason: reason)
        }
        return res
    }
}
...
/* Sorry, we really try hard to not use underscored attributes. In this case however we seem to break the inlining threshold which makes a system call take twice the time, ie. we need this exception. */
@inline(__always)
internal func wrapErrorIsNullReturnCall<T>(where reason: String = #function, _ body: () throws -> T?) throws -> T {
    while true {
